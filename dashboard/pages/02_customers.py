...
def load_data():
    ...
    amount_cols = [col for col in df.columns if any(x in col.lower() for x in ['amount', 'revenue', 'price'])]
    if amount_cols:
...
def main():
    ...
    with col4:
        ...
        latest_month_new = new_customers[new_customers['month'] == new_customers['month'].max()].shape[0]
    ...
    with col2:
        ...
        fig.update_layout(title='Revenue by Segment', xaxis_title='Revenue ($)', yaxis_title='Segment')
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        heatmap_pivot = heatmap_data.pivot(index='r_score', columns='f_score', values='count').fillna(0)
    ...
    with col2:
        ...
        fig.update_layout(title='Top 10 Customers by LTV', xaxis_title='LTV ($)', yaxis_title='Customer')
        st.plotly_chart(fig, use_container_width=True)
    ...
    segment_stats.columns = ['Customers', 'Avg Recency (days)', 'Avg Frequency', 'Avg LTV', 'Total Revenue']
