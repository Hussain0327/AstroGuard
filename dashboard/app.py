...
def load_data():
    ...
    try:
        ...
        df = pd.DataFrame({
            ...
            'channel': np.random.choice(['Organic', 'Paid', 'Email', 'Social', 'Referral'], n_records),
            'product_category': np.random.choice(['Electronics', 'Clothing', 'Home', 'Books', 'Other'], n_records)
        })
    ...
    # Find amount column
    amount_cols = [col for col in df.columns if any(x in col.lower() for x in ['amount', 'revenue', 'price', 'total'])]
    if amount_cols:
...
def create_channel_performance(df):
    ...
    if 'channel' not in df.columns:
        df['channel'] = np.random.choice(['Organic', 'Paid', 'Email', 'Social', 'Referral'], len(df))
...
def create_monthly_comparison(df):
    ...
    colors = ['#28a745' if x >= 0 else '#dc3545' for x in monthly['growth'].fillna(0)]
...
def main():
    ...
    with col1:
        delta = ((metrics['total_revenue'] - prev_metrics['total_revenue']) /
    ...
    with col2:
        delta = ((metrics['total_transactions'] - prev_metrics['total_transactions']) /
                 prev_metrics['total_transactions'] * 100) if prev_metrics['total_transactions'] > 0 else 0
        st.metric(
            label="Transactions",
    ...
    with col3:
        delta = ((metrics['avg_order_value'] - prev_metrics['avg_order_value']) /
                 prev_metrics['avg_order_value'] * 100) if prev_metrics['avg_order_value'] > 0 else 0
        st.metric(
    ...
    with col4:
        delta = ((metrics['unique_customers'] - prev_metrics['unique_customers']) /
                 prev_metrics['unique_customers'] * 100) if prev_metrics['unique_customers'] > 0 else 0
        st.metric(
    ...
    display_df = filtered_df[['date', 'customer_id', 'amount']].copy() if 'customer_id' in filtered_df.columns else filtered_df[['date', 'amount']].copy()
